...
void led_control(void *arg)
{
    ...
    while (1)
    {
        if (xQueueReceive(detection_queue, &detection_state, portMAX_DELAY)) //Control LED base on detection state
        {
        ...
        // ESP_LOGI("Read stack size", "High water mark: %d", stack_high_water_mark);
    }
}
    ...
    while (1)
    {
        ...
        if (xQueueReceive(button_intr, &request, portMAX_DELAY) == pdPASS)
        {
            if (request)
            {
                ...
                timer_init(); // Run calibration and start timer
            }
        }
        // UBaseType_t stack_high_water_mark = uxTaskGetStackHighWaterMark(NULL);
        // ESP_LOGI("Read stack size", "High water mark: %d", stack_high_water_mark);
...
{
    bool request = true;
    xQueueSendFromISR(button_intr, &request, NULL); // send interrupt signal to task
}
...
void adjust_brightness(void *arg) // Adjust brightness base on ADC value
{
    ...
    while (1)
    {
        ...
        // Remove the highest and lowest of array
        int sum = 0;
