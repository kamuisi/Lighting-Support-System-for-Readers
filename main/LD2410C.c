...
void send_data(const uint8_t *command, size_t command_length)
{
    ...
    // cat header, command, tail into 1 package
...
void read_data(void *arg)
{
    ...
    {
        uint8_t byte;
        int length = uart_read_bytes(UART_NUM_2, &byte, 1, 100 / portTICK_PERIOD_MS); // read data by byte
        if (length > 0)
        {
            ...
            else
            {
                ...
                if(index >= END_LENGTH)
                {
                    if (is_ack == false && memcmp(&data[index - END_LENGTH], End_of_frame, END_LENGTH) == 0)
                    {
                        ...
                        index = 0;
                        header_found = false; // already to read new frame
                    }
...
void noise_reduce(void *arg) // calibrate enviroment noise
{
